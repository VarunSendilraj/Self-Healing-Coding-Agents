...
Your response must be parseable by JSON.parse() without any preprocessing."""
    ...
    def classify_failure(
        ...
        """
        logger.info("🤖 LLM FAILURE CLASSIFICATION: Starting LLM-based failure analysis...")
        ...
        try:
            ...
            logger.error(f"🚨 LLM FAILURE CLASSIFICATION ERROR: {e}")
...
"""
    ...
    def _log_classification_results(self, classification: Dict[str, Any]) -> None:
        ...
        logger.info(f"🎯 LLM CLASSIFICATION: {classification['primary_failure_type']}")
        logger.info(f"🎯 CONFIDENCE: {classification['confidence']:.2f}")
        logger.info(f"🎯 RECOMMENDED TARGET: {classification['recommended_healing_target']}")
